...
class StoryForgeLLM:
    ...
    def generate(self, prompt: str, model: str = "gpt-4o-mini", temperature: float = 0.7, max_tokens: int = 4000) -> str:
...
def generate_story_bible(llm: StoryForgeLLM, perspective: dict, parameters: dict, importance: dict) -> dict:
    """Generate a comprehensive story bible based on the perspective and user parameters
    
    This replaces the generate_flash_cards function with a more detailed approach that
    takes into account the user's preferences and importance weights.
...
Create a story bible with the following EXACT sections and format:
{{
...
def get_default_story_bible(perspective, parameters):
    ...
    return {
        ...
        "setting": f"A richly detailed {genre} world with unique cultures and landscapes",
...
def storyforge_pipeline(user_query: str) -> Dict[str, Any]:
    ...
    try:
        ...
        story_bible = generate_story_bible(llm, memory, flash_cards_list)
